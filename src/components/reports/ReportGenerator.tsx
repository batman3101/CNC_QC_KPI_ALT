...
export function ReportGenerator({ machines, models }: ReportGeneratorProps) {
    ...
    <Card elevation={3}>
      ...
      <CardContent>
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
          ...
          {/* Machine and Model Filters */}
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, md: 6 }}>
              <FormControl fullWidth>
                <InputLabel>{t('reports.machine')}</InputLabel>
                <Select
                  value={machineId}
                  onChange={(e: SelectChangeEvent) => setMachineId(e.target.value)}
                  label={t('reports.machine')}
                >
                  <MenuItem value="">{t('reports.allMachines')}</MenuItem>
