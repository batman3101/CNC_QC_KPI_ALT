...
5. **기존 데이터 정리**(승인 필요, 미수행): 쌍둥이 15쌍은 검사 수량 5,998개가 이중 집계된 상태다. 어느 쪽을 남길지, 63건 무쌍둥이 누락에 불량 기록을 만들지는 현장 확인 후 결정한다. 기록 부재만으로 자동 생성하지 않는다.

주의: 마이그레이션이 운영 DB에 적용되기 전에 이 코드를 배포하면 저장이 전부 실패한다(함수 없음). 반대로 마이그레이션만 먼저 적용하는 것은 안전하다. 옛 번들을 아직 실행 중인 기기는 기존 두 요청 경로를 계속 쓰므로, 번들이 갱신될 때까지 누락·중복이 더 생길 수 있다.
