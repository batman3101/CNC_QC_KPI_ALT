...
1. 관리자로 로그인하고 권한 관리에서 공장을 선택합니다.
2. 매니저/검사자의 기능 스위치를 변경하고 저장한 뒤 성공 메시지를 확인합니다.
3. 해당 역할로 다시 로그인하여 허용 메뉴만 표시되고, 차단 URL 직접 접근도 거부되는지 확인합니다.
4. 다른 브라우저에서 관리자가 권한을 변경했을 때 메뉴와 접근 권한이 실시간으로 갱신되는지 확인합니다.
5. Supabase 연결/RPC를 차단했을 때 권한이 열리지 않는(fail-closed)지 확인합니다.
6. 매니저에게 `관리` 접근을 허용해도 공장 구분이 없는 공용 기준정보는 조회만 가능하고, 생성·일괄 업로드·수정·삭제 버튼이 표시되지 않는지 확인합니다.
7. `사용자 관리` 권한이 없는 계정이 `users` 원본 테이블의 다른 사용자 이메일을 읽지 못하고, 검사/분석 화면에서는 제한된 사용자 디렉터리의 이름·역할만 조회하는지 확인합니다.
8. 배포 직후 `supabase/tests/role_permissions_verification.sql`을 데이터베이스 소유자 권한으로 실행하고 오류 없이 `ROLLBACK`까지 완료되는지 확인합니다. 이 검증은 권한 변경을 한 번 수행하지만 동일 트랜잭션에서 모두 되돌립니다.
9. ALT와 ALV 설정이 서로 섞이지 않는지 확인합니다.
